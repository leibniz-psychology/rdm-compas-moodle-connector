...
class Eb_Shortcode_User_Profile {
	...
	public static function get_errors( $posted_data ) {
		...
		$required_fields = apply_filters(
			...
			array(
				'username' => __( 'Username', 'rdmcompas-moodle-connector' ),
		...
		foreach ( $required_fields as $field_key => $field_name ) {
			if ( empty( $posted_data[ $field_key ] ) ) {
				...
				$errors[] = sprintf( __( '%$1s is required field.', 'rdmcompas-moodle-connector' ), '<strong>' . $field_name . '</strong>' );
			}
		}
		...
		if ( ! is_email( $email ) ) {
			/* Translators 1: email */
			$errors[] = sprintf( __( '%$1s is invalid email.', 'rdmcompas-moodle-connector' ), '<strong>' . $email . '</strong>' );
		} elseif ( email_exists( $email ) && $email !== $current_user->user_email ) {
			/* Translators 1: email */
			$errors[] = sprintf( __( '%$1s is already exists.', 'rdmcompas-moodle-connector' ), '<strong>' . $email . '</strong>' );
		}

		$username = sanitize_user( $posted_data['username'] );
		if ( username_exists( $username ) && $username !== $current_user->user_login ) {
			/* Translators 1: User name */
			$errors[] = sprintf( __( '%$1s is already exists.', 'rdmcompas-moodle-connector' ), '<strong>' . $username . '</strong>' );
		}
