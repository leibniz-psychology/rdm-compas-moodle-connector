...
class Eb_Shortcode_My_Courses {
	...
	public function show_my_courses( $my_courses, $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			<p>
				...
				printf(
					esc_html__( 'You are not logged in. ', 'rdmcompas-moodle-connector' ) . '%s' . esc_html__( ' to login.', 'rdmcompas-moodle-connector' ),
					"<a href='" . esc_url( \app\wisdmlabs\edwiserBridge\wdm_eb_user_account_url() ) . "'>" . esc_html__( 'Click here', 'rdmcompas-moodle-connector' ) . '</a>'
				);
				?>
			</p>
			<?php
			...
			<h5>
				...
				printf(
					/* Translators 1: URL */
					esc_html__( 'You are not enrolled to any course. ', 'rdmcompas-moodle-connector' ) . '%s' . esc_html__( ' to access the courses page.', 'rdmcompas-moodle-connector' ),
					"<a href='" . esc_html( $link ) . "'>" . esc_html__( 'Click here', 'rdmcompas-moodle-connector' ) . '</a>'
				);
				?>
			</h5>
			<?php
	...
	private function get_course_progress( $course_id, $progress_data, $user_id, $attr, $moodle_user_id ) {
		...
		$progress_meta_data = __( 'Not available', 'rdmcompas-moodle-connector' );
		$completed          = 0;
		...
		$btn_text           = __( 'Start', 'rdmcompas-moodle-connector' );
		ob_start();
		?>
		<div class='eb-course-action-cont'>
			...
			if ( $is_user_suspended ) {
				...
				$btn_text       = __( 'Suspended', 'rdmcompas-moodle-connector' );
			} elseif ( in_array( $course_id, $course_ids ) ) {// @codingStandardsIgnoreLine.
				if ( 0 === $progress ) {
					$progress_class     = 'eb-course-action-btn-start';
					$btn_text           = __( 'Start', 'rdmcompas-moodle-connector' );
					$progress_meta_data = __( 'Not yet started', 'rdmcompas-moodle-connector' );
				} elseif ( $progress > 0 && $progress < 100 ) {
					$progress_class     = 'eb-course-action-btn-resume';
					$btn_text           = __( 'Resume', 'rdmcompas-moodle-connector' );
					$progress_meta_data = esc_attr( round( $progress ) ) . __( '% Completed', 'rdmcompas-moodle-connector' );
				} else {
					$completed          = 1;
					$progress_class     = 'eb-course-action-btn-completed';
					$btn_text           = __( 'View', 'rdmcompas-moodle-connector' );
					$progress_meta_data = __( '100% Completed', 'rdmcompas-moodle-connector' );
				}
			}
			?>
		...
		if ( $courses->have_posts() ) {
			...
			*/
			...
			<a href="<?php echo esc_html( $view_more_url ); ?>" class="wdm-btn eb-rec-courses-view-more">
				<?php esc_html_e( 'View More &rarr;', 'rdmcompas-moodle-connector' ); ?>
			</a>
			<?php
		} else {
			$template_loader->wp_get_template_part( 'content', 'none' );
	...
	public function generate_recommended_courses() {
		...
		$attr['recommended_courses_wrapper_title'] = esc_html__( 'Recommended Courses', 'rdmcompas-moodle-connector' );
