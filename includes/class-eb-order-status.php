...
class Eb_Order_Status {
	...
	public function save_new_order_place_note( $order_id ) {
		...
		$msg        = esc_html__( 'New order has been placed for the ', 'rdmcompas-moodle-connector' ) . '<strong>' . sprintf( '%s', get_the_title( $course_id ) ) . '</strong>' . esc_html__( ' course.', 'rdmcompas-moodle-connector' );
	...
	private function unenroll_user_from_courses( $order_id ) {
		...
		if ( $resp ) {
			...
			$note     = array(
				...
				'msg'  => __( 'User enrollment has been suspended on order refund request.', 'rdmcompas-moodle-connector' ),
			);
			\app\wisdmlabs\edwiserBridge\wdm_eb_update_order_hist_meta( $order_id, $cur_user->user_login, $note );
		}
		return $resp;
